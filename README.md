...
| 模块 | 状态 | 说明 |
|------|------|------|
| 全景虚拟漫游 | ✅ 可用 | 基于 [Pannellum](https://pannellum.org/)，多场景热点跳转（走廊、大厅、楼梯、二楼等） |
| 楼内路网数据 | ✅ 进行中 | B7 **1–5 层 × A/B 座** 共 10 份 `graph.json`，见 `node_nav/data/index.json` |
| 最短路径算法 | ✅ 可用 | Python `indoor_nav` + JavaScript `pathfind.js`（Dijkstra，无向带权图） |
| 设施最近查找 | ✅ 可用 | 按类型（洗手间、楼梯、电梯等）找最近节点并输出路径 |
| 二维地图页 | 🚧 计划中 | `map.html`：平面图 + 起终点 + 路径高亮（见分阶段方案文档） |
...
```
PanoramaProject/
├── server_main.py          # 本地 HTTP 服务（默认端口 8000）
├── panorama.html           # 全景漫游 + 导航面板
...
- 全景页：<http://localhost:8000/panorama.html>
